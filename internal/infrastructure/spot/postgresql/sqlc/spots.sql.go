// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.29.0
// source: spots.sql

package sqlc

import (
	"context"
)

const selectAllSpots = `-- name: SelectAllSpots :many
SELECT
    id,
    name,
    created_at
FROM
    web_spot
`

func (q *Queries) SelectAllSpots(ctx context.Context) ([]WebSpot, error) {
	rows, err := q.db.Query(ctx, selectAllSpots)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []WebSpot
	for rows.Next() {
		var i WebSpot
		if err := rows.Scan(&i.ID, &i.Name, &i.CreatedAt); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const selectSpotByName = `-- name: SelectSpotByName :one
SELECT id, name, created_at
FROM web_spot
WHERE lower(name) = lower($1)
LIMIT 1
`

func (q *Queries) SelectSpotByName(ctx context.Context, name string) (WebSpot, error) {
	row := q.db.QueryRow(ctx, selectSpotByName, name)
	var i WebSpot
	err := row.Scan(&i.ID, &i.Name, &i.CreatedAt)
	return i, err
}

const selectSpotsByNameLike = `-- name: SelectSpotsByNameLike :many
SELECT id, name, created_at
FROM web_spot
WHERE lower(name) LIKE lower($1)
ORDER BY name
LIMIT 15
`

func (q *Queries) SelectSpotsByNameLike(ctx context.Context, namePattern string) ([]WebSpot, error) {
	rows, err := q.db.Query(ctx, selectSpotsByNameLike, namePattern)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []WebSpot
	for rows.Next() {
		var i WebSpot
		if err := rows.Scan(&i.ID, &i.Name, &i.CreatedAt); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
